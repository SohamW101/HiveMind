...
class GreedyController:
    ...
    def reset(self):
        ...
        self.drop_cells = set()
        depot_grid = getattr(env, "depot_pos_grid", (0, 0))
        for dr, dc in ((-1, 0), (1, 0), (0, -1), (0, 1)):
            nr, nc = depot_grid[0] + dr, depot_grid[1] + dc
            if 0 <= nr < env.grid_size and 0 <= nc < env.grid_size and (nr, nc) not in self.blocked:
                self.drop_cells.add((nr, nc))
    ...
    def _can_pick(self, i, slot):
        """Must be Manhattan distance 1 from carton cell (cardinal neighbor)."""
        rx, ry, _ = self.env._canonical_pose(i)
        robot_cell = self.env._world_to_grid(rx, ry)
        carton_cell = self._carton_cell(slot)
        return abs(robot_cell[0] - carton_cell[0]) + abs(robot_cell[1] - carton_cell[1]) == 1

    def _within_reach(self, i, cell):
        """Is `cell`'s centre within Manhattan distance 1 of robot i?"""
        rx, ry, _ = self.env._canonical_pose(i)
        robot_cell = self.env._world_to_grid(rx, ry)
        return abs(robot_cell[0] - cell[0]) + abs(robot_cell[1] - cell[1]) <= 1

    def _can_drop(self, i):
        """Must be Manhattan distance 1 from depot cell (cardinal neighbor)."""
        rx, ry, _ = self.env._canonical_pose(i)
        robot_cell = self.env._world_to_grid(rx, ry)
        depot_cell = getattr(self.env, "depot_pos_grid", (0, 0))
        return abs(robot_cell[0] - depot_cell[0]) + abs(robot_cell[1] - depot_cell[1]) == 1

    def _neighbourhood(self, cell):
        """Cells from which `cell` is reachable - 4 cardinal neighbours."""
        r, c = cell
        out = set()
        for dr, dc in ((-1, 0), (1, 0), (0, -1), (0, 1)):
            n = (r + dr, c + dc)
