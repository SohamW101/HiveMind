...
The script saves a checkpoint every `--checkpoint-every` steps (default 250k) to the `models/checkpoints/`
directory, and a final model to `models/<run_name>_final.zip`.
To resume training from a checkpoint, use `--init-from models/checkpoints/<checkpoint_name>.zip`.
This will warm-start the policy weights from the checkpoint. Note that the SB3 optimizer state is not saved
in this configuration, so the learning rate schedule will restart from `--lr`.
"""
...
def build_env(
    ...
    return VecNormalize(VecMonitor(vec), norm_obs=False, norm_reward=True, gamma=gamma)
...
def main():
    ...
    p.add_argument(
        ...
        type=int,
        default=4096,
        help="Mini-batch size. Default 4096 is optimized for RTX A5000 (24GB VRAM) "
        "to maximize throughput. If running on a smaller GPU, reduce this.",
    )
    ...
    if tb_log is None:
        print(
            "  [warn] tensorboard is not installed - training will run, but no "
    ...
    print("=" * 78)
    print(
        "  HiveMind - shared-policy PPO"
        + (" + emergent communication" if args.communication else ", no communication")
        + " (roadmap step "
        + ("7" if args.communication else "6")
        + ")"
    )
    print("=" * 78)
    print(f"  run          : {run_name}")
    print(
        f"  worlds       : {worlds}  ->  {slots} policy slots ({NUM_AGENTS} per world)"
    )
    print(f"  observation  : {DEFAULT_OBS_DIM} floats per robot")
    print(f"  timesteps    : {args.timesteps:,} robot-steps")
    print(
        f"  rollout      : {args.n_steps} per slot  ->  buffer {args.n_steps * slots:,}"
    )
    ...
    print(
        f"  max steps    : {args.max_steps or f'{max_steps_for(args.num_cartons)} (per carton count)'}"
    )
    ...
    model = RecurrentPPO(
        ...
        gae_lambda=0.95,
        clip_range=0.2,
        ent_coef=0.02,  # Higher entropy prevents premature convergence on promotion
        vf_coef=0.5,
        max_grad_norm=0.5,
    ...
    if args.init_from:
        ...
        donor = _PPO.load(
            args.init_from, device=device, custom_objects=INFERENCE_CUSTOM_OBJECTS
        )
    ...
    print(f"\n  saved      : {final}")
    print(
        f"  wall clock : {elapsed / 60:.1f} min for {steps:,} robot-steps "
