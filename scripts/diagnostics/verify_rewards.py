...
class Driver:
    ...
    def act(self, action, note=""):
        ...
        if self.trace:
            ...
            print(
                f"  step {self.env.current_step:4d} | {ACTION_NAMES[action]:6s} | "
...
def main():
    ...
    print(
        f"  delivery +{R_PER_DELIVERY} shared / +{R_OWN_DELIVERY} own   "
        f"pickup +{R_OWN_PICKUP} own   all-delivered +{R_ALL_DELIVERED}"
    )
    print(
        f"  time {R_TIME_PENALTY}/step   idle {R_IDLE_PENALTY}/step   "
    ...
    try:
        ...
        expected = [
            SHARED_WEIGHT * b["shared_total"] + INDIVIDUAL_WEIGHT * iv + f
            for iv, f in zip(b["individual_totals"], shaping_i)
        ]
        check(
            "R_total_i = 0.90*shared + 0.10*individual_i + F_i for all four agents",
            all(abs(e - r) < TOL for e, r in zip(expected, rew_drop)),
            f"expected {expected}\n        got      {list(rew_drop)}",
        )
        check(
            "shaping is NOT inside the individual total (it would be diluted 10x)",
            all("shaping (unweighted)" not in str(k) or True for k in [0])
            and all(
                abs(sum(v for k, v in t.items() if k != "shaping (unweighted)") - iv)
                < TOL
        ...
        check(
            "PICKUP PAYS: the picking agent's total reward is positive",
            rew_pickup[0] > 0,
        ...
        check(
            "DELIVERY PAYS: the delivering agent's total reward is positive",
            rew_drop[0] > 0,
            f"delivery step paid {rew_drop[0]:+.4f}",
        )
        ...
        b = info["reward_breakdown"]
        check(
            "dropping while empty is invalid",
        ...
        b = info["reward_breakdown"]
        check(
            "grabbing at nothing is invalid",
            b["individual_terms"][0].get("invalid_action") == R_INVALID_ACTION,
            f"got {b['individual_terms'][0]}",
        )
        check(
            "invalid action is individual, never shared",
    ...
    try:
        ...
        b = info["reward_breakdown"]
        print(
            f"  contact pairs: {info['collision_pairs']}   "
            f"shared terms: {fmt_terms(b['shared_terms'])}"
        )
        check(
            "driving into another robot registers a collision",
            info["collisions"] >= 1,
            f"collisions={info['collisions']}",
        )
        check(
            "collision charges -5.0 shared",
        ...
        shared_part = [
            r - INDIVIDUAL_WEIGHT * iv - t.get("shaping (unweighted)", 0.0)
            for r, iv, t in zip(rew, b["individual_totals"], b["individual_terms"])
        ]
        check(
            "collision is shared, so all four agents pay the same amount",
            max(shared_part) - min(shared_part) < TOL,
            f"shared components differ: {[round(v, 6) for v in shared_part]}",
        )

        _, _rew2, _, _, info2 = env.step([6, 6, 6, 6])
        check(
            "a pair that stays overlapped is not re-charged every step",
    ...
    try:
        ...
        b = info["reward_breakdown"]
        print(
            f"  pose {tuple(round(v, 2) for v in before)} -> "
        ...
        check(
            "the shelf row is in the blocked set",
        ...
        check(
            "carton gap cells stay walkable",
    ...
    try:
        ...
        check(
            "all-delivered bonus is +100.0",
            b["shared_terms"].get("all_delivered") == R_ALL_DELIVERED,
        )
        check(
            "makespan bonus matches 50*(T_max-T_actual)/T_max",
        ...
        check(
            "final delivery still pays its +10 shared and +2 own",
            b["shared_terms"].get("per_delivery") == R_PER_DELIVERY
    ...
    try:
        ...
        check(
            "no completion bonus on a truncated episode",
