...
class Stats:
    ...
    def conversion(self):
        return (
            self.converted / self.opportunities if self.opportunities else float("nan")
        )
...
def report(label, st):
    ...
    if st.opportunities == 0:
        print(
            f"     opportunities  0  <- never got a carton within reach in "
    ...
    omix = "  ".join(
        f"{n} {100.0 * c / om:.0f}%"
        for n, c in zip(ACTION_NAMES, st.out_range_actions)
        if c
    )
    print(f"     for contrast, out of range    : {omix}")
    print(
        f"     PICKUP pressed with nothing in reach: {st.pickup_out_of_range}"
        f"  (costs -0.5 individual each)"
    )
    print(
        f"     pickups {st.pickups / st.episodes:.1f}/ep   "
...
def verdict(det, sto):
    ...
    if d < 0.05 and s >= 0.20:
        ...
        print(
            "  though the robot is standing in range. The reward pays ~+10 for it, so"
        )
...
def main():
    ...
    label = (
        f"{cartons if cartons else 12} cartons, {args.episodes} episodes "
    ...
    if args.baseline:
        ...
        print("=" * 78)
        st = run(
            None, "greedy", args.episodes, cartons, deterministic=True, seed0=args.seed0
        )
        report("greedy", st)
        return
    ...
    print("=" * 78)
    det = run(
        model, "policy", args.episodes, cartons, deterministic=True, seed0=args.seed0
    )
    report("deterministic (argmax)", det)
    sto = run(
        model, "policy", args.episodes, cartons, deterministic=False, seed0=args.seed0
    )
